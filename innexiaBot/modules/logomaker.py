...
async def lego(event):
 ...
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="💕 Made By @MISS_EMMA_ROBOT Support @HUNTER_IS_BACK")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @HUNTER_IS_BACK, {e}')
...
async def lego(event):
 ...
 try:
    ...
    fname2 = "LogoByInnexia.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @MISS_EMMA_ROBOT")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @HUNTER_IS_BOT, {e}')
